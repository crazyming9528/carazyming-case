...
var my,timer=null,timers=null,timerss,progress,_appstatus,open=false,xd=true,xdtime=false,xdt=false;//定义使用的变量;
var timeArr={};
...
function getInfoGame() {
    $.get($xd.gameUrl+'/'+6024,{},function (res) {
        console.log(res)
    ...
    callAppFunc_dbGet(game_id,function (json) {
        ...
        if (status){//没有下载过
            ...
            if (localStorage.getItem('ru') == '1'){
                ...
                a.status=0;
                callAppFunc_dbSet(game_id,a);
                callAppFunc_startDown(game_id)
                //定时器取状态
                pasuedDown()
            } else if (localStorage.getItem('ru') == '4'){
                open=true;//开关变量
                xd=false;
                svgBg.style.backgroundImage="url(./img/paly.png)";
                svgBg1.style.backgroundImage="url(./img/paly.png)";
